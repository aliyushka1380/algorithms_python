...
def check_1(lst_obj):
    ...
    Сложность: O(1000).
    """
    lst_to_set = set(lst_obj)  # O(1000) - линейная сложность (len(check_1(lst)))
    return lst_to_set
...
def check_2(lst_obj):
    ...
    for j in range(len(lst_obj)):          # O(N)
        if lst_obj[j] in lst_obj[j+1:]:    # O(N) + O(N) - квадратичная - наихудшая оценка
            return False                   # O(1)
    return True                            # O(1)
...
def check_3(lst_obj):
    ...
    lst_copy.sort()                          # O(N Log N) - линейно-логарифмическая - наихудшая оценка
